...
Blockly.Toolbox.buildTree_ = function() {
  console.log("RAM: building Tree of blocks with 'category'");
  var tree = {};
...
Blockly.Toolbox.redraw = function() {
  ...
  console.log("RAM: redrawing Toolbox, 1 option per category");
  var options = [];
  for (var cat in Blockly.Toolbox.languageTree) {
    ...
    option.cat = cat;
    options.push(option);
    console.log("RAM: redraw, option = " + option.text + " cat = " + option.cat);
  }


  // RAM: Don't include blockly variables and procedures
  var option = {};
  ...
  if (Blockly.Language.procedures_defnoreturn ||
    ...
    //    console.log("RAM: redraw, option = " + option.text);
  }
